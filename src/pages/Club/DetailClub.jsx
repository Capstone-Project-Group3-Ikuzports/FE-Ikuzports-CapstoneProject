...
const DetailClub = () => {
	...
	const getDetailClub = async () => {
		await axios
			.get(`https://rubahmerah.site/clubs/${id}`, config)
			.then((response) => {
				setLoading(true);
	...
	const onSubmitHandler = async () => {
		...
		await axios
			.post(`https://rubahmerah.site/members`, form, config)
			.then((response) => {
				Swal.fire({
					position: "center",
					icon: "success",
					...
					<Box w={"70%"}>
						<Flex>
							...
							<Box mt={10} ml={20}>
								...
								<Text fontSize={"3xl"} mt={5} ml={2}>
									Member : {getClubDetail.joined_member} /{" "}
									{getClubDetail.member_total}
								</Text>
								<Text fontSize={"3xl"} mt={5} ml={2}>
									Category : {getClubDetail.category_name}
								</Text>
								<Text fontSize={"3xl"} mt={5} ml={2}>
									{getClubDetail.city}, {getClubDetail.address}
								</Text>
							</Box>
						</Flex>
						...
						<Text fontSize={"xl"} mt={"2"} w={"60%"}>
							{getClubDetail.description}
						</Text>
					</Box>
					<RuleCard key={getClubDetail.id} rule={getClubDetail.rule} />
				</Flex>
